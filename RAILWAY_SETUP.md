...
- **Start Counting in VC text channel** (`startcount` / `Startcount` / `START COUNT`):
  ...
  - Works both with and without the configured prefix.
- **Clear VC counts command** (`clearcount <number>` / `countclear <number>`):
  - Deletes the last `<number>` counting messages sent by the bot **inside the voice channel's text chat** (VC text channel) and rolls back the internal count.
  - Works both with and without the configured prefix.

---

## 🔒 VC Chat Command Support
If you send the command `<prefix>clearcount <number>` or `<prefix>countclear <number>` directly inside the **connected voice channel text chat**, the bot will delete `<number>` of its recent counting messages from that voice chat and roll back the count.
